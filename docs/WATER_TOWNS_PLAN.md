...
| 2 | Canal city: basin + canal spokes + bank-hug cost + density variant + grass-floor tests, behind `/debug/tileset`. **BUILT 2026-07-05 (debug-only, pending review):** `placeCanalsInTown` in `townMapGenerator.js` (opt-in via `water.archetype: 'canal'`, CITY size + coastal context only, smaller sizes / lake contexts fall back to plain generation): 2x2 harbour basin 3-4 tiles off the square on the seaward side (never bridged: it is a pool), one guaranteed basin-to-sea channel plus a seeded second outlet that fences off a district, 2-4 interior spokes (3-5 total, budget-unified with the sea at ~25% of the core), all `waterway: true` width-1 windy channels carved before roads/buildings; `CANAL_BANK_COST` (0.8) quay pull in `routeWindyPath` (opt-in via `bankCost`, non-canal routing byte-identical); `CANAL_BUILDING_CONFIG` density variant (houses 55 -> 42, stables/mill dropped); reserved quest-grass floors (10 spread core tiles + 4 grass per fenced-off district) protected from paving/building/decoration; `boathouse` building type (bank-moored, STEP 1.7) with art/legend/emoji + Boatwright staffing in `npcGenerator`; canal-city seed-survey tests (40 seeds x 4 coasts) + coastal-city fixture pin in `townMapGenerator.test.js`; canal-city preset + stats + basin/district overlay on `/debug/tileset`. Canals render as plain water in this cut (Phase 4 art). | L | 1 (shares plumbing) |
| 3 | World-gen shim (`riverToSea`, `estuaryTown` options) + `waterTown` stamp in NewGame + `useGameMap` pass-through. **BUILT 2026-07-05 (pending review):** two additive `generateMapData` options (default off, fixture-pin byte-identical): `riverToSea` steers the FIRST river to the nearest non-lake water (survey: 60/60 seeds reach the coast vs 34/60 legacy) and `estuaryTown` places the first town (one slot of the normal budget) on a dry tile at the river's sea mouth, pins it to CITY size (size-swap before naming, so size-tagged custom names still hold), and excludes it from starting-town selection. Stamping lives in `src/game/waterTowns.js` and runs ONCE in `campaignLauncher.launchCampaign` via `resolveWaterTownAccess()` (canUseWaterTown per archetype): premium worlds stamp `waterTown: 'canal'` on the first non-starting coastal river CITY (lagoon fallback to a pure-coast city per section 9 question 1; measured 150/150 canal placement, 88.7% true estuary, max 1 enforced), member+ worlds stamp `waterTown: 'riverfork'` on a seeded ~50% roll (measured 54.2%) over eligible river settlements (town/city carrying `hasRiver`); free/guest worlds get no options and no stamps (byte-identical, tested). Pass-through: `townWater.getTownWaterContext` merges the stamp into the water context as `water.archetype` for the launcher's town pre-generation and all three `useGameMap` lazy call sites (cached towns untouched). New Game shows a small tier-gated note (absent for free, decision 13) and its manual map preview uses the same options. Tests: shim surveys + estuary pins in `mapGenerator.test.js`, stamping/tier-flow/injection suite in `waterTowns.test.js`, context merge in `townWater.test.js`. | M | 1-2 |
| 4 | Art polish: canal water variant, quay autotiling, boathouse art/emoji/legend + Boatwright staffing. **BUILT 2026-07-05 (pending review):** directional canal tiles in `townTileArt.js` (`canalTile(mask, theme, seed)` keyed off the 4-bit waterway-neighbour mask, wall-autotiler technique: straights, bends, T-junctions, cross, mouths/ends, walled pool; adjacent basin/sea water AND bridge tiles count as wet, so mouths render open and the channel continues under bridges); calmer/darker canal water; basin treatment derived from the same mask (3-4 wet neighbours = open harbour water, bank only on dry edges, corner nibs); bridge-over-canal (deck oriented across the channel from the water neighbours, water visible beneath; plain bridges byte-identical); quay lips (subtle stone band + bollards) on path tiles beside waterway water; theme bank masonry (stone / desert sandstone / snow ice-grey); `waterwayMask()` wired through `TownMapDisplay` + `tileBackground` as an additive `waterMask` arg (default 0), temperate non-canal pins stay byte-identical (pin-tested); Canal legend swatch; full canal vocabulary gallery (16 masks + bridge + quay swatches) on `/debug/tileset`; river-fork water picks the treatment up for free via the shared `waterway` flag. Boathouse art/emoji/legend + Boatwright staffing had already landed with Phase 2. | S-M | 1-2 |
| 5 | Premium packaging: `canUseWaterTown`, flagship template in `premiumTemplates.local.js`, locked NewGame card | S | 3 |
| 6 | Side-quest flavor (smuggling/ferry/toll) + end-to-end verification on live saves | S | 3-5 | **BUILT 2026-07-06 (pending review; live-save verification remaining).**

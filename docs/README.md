...
- `AI_QUEST_GENERATION_PLAN.md`: AI-authored campaigns/quests with engine-typed deterministic milestones (#77; proposed 2026-07-16). AI fills validated authored slots via a staged pipeline (no zero-shot), gated by the content audit; Members+ gate on the Freeform generator shipped 2026-07-16; the interim Freeform (AI-generate) tab itself was then removed from NewGame 2026-07-18, pending the staged rebuild this plan describes. Rests on the #76 "engine referees" contract
...
- `COMBAT_UX_PLAN.md`: combat presentation rework (#79; brainstorm 2026-07-16). Combat animations (slice of #78), refining the clunky ~1440-line encounter modal, and an optional inline "open-play" rework; the keystone `useEncounterFight` extraction plus the Thread B modal refine shipped 2026-07-17/18, animations and the open-play spike remain. Mechanics unchanged (owned by `ENCOUNTER_SYSTEM.md`)
...
- `MAP_LAYOUT_PLAN.md`: promote the map from modal to main stage (#84; proposed 2026-07-17, HIGH PRIORITY). The map modal hosts the core loop but hides the narration log it feeds; leaning a two-pane workspace (docked map + persistent Adventure Log, tabs on mobile) that dissolves the reopen-around-encounters churn and converges with #79's open-play stage
- `SKILL_CHECK_PLAN.md`: engine-rolled skill checks for narrative actions, BG3-style (#83; Phases 1-2 shipped 2026-07-20: `src/game/skillCheck.js` + persisted check-lock ledger; Phases 3-4 open). Model proposes `[CHECK: skill, tier]` (bounded judgment), engine rolls vs the `DIFFICULTY_DC` ladder with full modifier parity, LLM narrates the handed result; failure always changes world state (fail-forward / complication / persisted lockout / escalation) and never dead-ends the golden path. Extends #76 and resolves its open question 5

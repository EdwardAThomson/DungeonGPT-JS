...
| `GET /api/content/templates`, `GET /api/content/templates/:id` (**new** route group for #40, e.g. `cf-worker/src/routes/content.ts`) | List returns all entries with premium ones as **locked stubs** (id, name, icon, description, `premium: true`, no `settings`/`milestones`). Detail returns full template JSON only if entitled, else 403 `premium_required`. Premium template JSON lives in a `premium_templates` table (or Worker-bundled private data), never in the public repo. Future music: short-lived signed R2 URLs behind the same check. |

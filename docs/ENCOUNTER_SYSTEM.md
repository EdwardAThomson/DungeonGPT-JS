...
**Wandering monsters spawn as visible mobs (sites only).** A site's per-step wandering roll used to open a combat modal out of nowhere: the monster was invisible, with no icon, threat ring, or chance to flee. Now an immediate-tier site wandering roll instead SPAWNS a visible, already-aggro'd mob a few tiles from the party (`spawnWanderingMob` in `mobMovement.js`, pure + testable): it picks a walkable, unoccupied, in-band (`WANDERING_SPAWN_MIN_DIST 2` / `_MAX_DIST 4` Manhattan), party-reachable tile, capped at `WANDERING_MOB_CAP` (2) live wandering mobs per site. The mob renders with its threat ring, chases via the same `stepMobs` path, and engages on contact like any placed mob, so the party can assess and flee it. Sites only: the mob/entity layer exists only in sites, so world-map and town wandering rolls are untouched and still use the modal.
...
- **Town rate is tuned separately (2026-07).** The pity/streak bonus (+0.10 at moves>=3, +0.15 at moves>=5) is an absolute add that dwarfs the tiny town base, so it is gated behind `!isTown` (towns roll at a flat base while world-map and site biomes keep the pity bonus). Because a town walk moves tile-by-tile, `Game.js` `runTownStep` also rolls the town encounter only on every 3rd entered tile (0, 3, 6, …) so a multi-tile crossing doesn't compound to a near-certain fight (a 6-tile crossing drops from ~59-87% to ~13%). Site and world-map rolling are unchanged.

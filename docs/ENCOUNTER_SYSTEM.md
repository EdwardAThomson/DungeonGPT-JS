...
| `src/utils/inventorySystem.js` | Gold management, item drops, dice notation rolling, `consumeHealingItem` (shared heal path) |
...
- **In-combat item use.** A "Use Item" action opens a fixed picker of the party's healing consumables + a target hero and heals through the shared `consumeHealingItem` path. In a multi-round fight it consumes the party's action for that round (no attack, no enemy-HP progress, spends a round against the cap); a single-round fight just heals + consumes. Items are finite, so this is never a free infinite heal.

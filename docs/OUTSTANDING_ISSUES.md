...
| 52 | **Unified tabbed modal hub ("Adventure Book").** Converge the REFERENCE modals (Journal, Party/Inventory, Codex #51, Settings) into one tabbed hub — open once, hop between tabs, remembered last-tab, one hotkey. Transactional interrupts (encounter/dice/building/save/offer) stay separate modals with conflict rules. Map joins ONLY if click-to-move-while-open survives as a tab, else stays standalone. Prerequisite: finish the ModalContext migration (Settings/Map/Building still boolean-state). | Design chat 2026-07-03 | M-L | |
| 53 | **Re-sim the t3 QUEST_ENEMIES band.** The five t3 registry entries sim OUTSIDE the healthy band under lint semantics (e.g. elder_wyrm as-registered: 18% win / 65% TPK for 3-hero mid Lv6 — the t3 flagship's inline copy was softened to 44%/7%). They're configurator-only today (side quests don't pull t3 yet) so nothing fails, but retune HP/damage/DC before they surface. | t3 authoring 2026-07-03 | S | |

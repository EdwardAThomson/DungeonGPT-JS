...
| 38 | **Site-map crystal decorations read as pickable loot.** `siteTileArt` sprinkles 💎 "Crystals" decorations on cave/site floors; players try to pick them up (actual loot is the 💰 tile, claimed by walking onto it). Options: make crystal-decorated tiles harvestable (nice: minor loot roll), swap the deco to something non-item-like, or a legend/tooltip. | Playtest 2026-07-02 (mountain site) | S | |
...
| 41 | **Journal: side-quest tab.** Move side quests out of the below-milestones section into a dedicated Journal tab (📜 Campaign | 🗺️ Side Quests (n)), cards sorted ready-to-turn-in → active → completed; integrate with the modal's existing journal/settings view switch. Design: [SIDEQUEST_UX_PLAN.md](SIDEQUEST_UX_PLAN.md). | Playtest 2026-07-03 | S-M | |
| 42 | **Side-quest objective clarity.** Per-step progress ("2/3") + derived how/where hints (site steps: "in a cave — marked on your map"; gathers: sources computed from shopStock/LOOT/encounter tables via describeItemSources; bounties: "any victory counts"; turn-ins: readiness + where), and an accept-time "🗺️ A cave has been revealed on your map" toast. Mechanics all exist — this is the missing communication layer. Design: [SIDEQUEST_UX_PLAN.md](SIDEQUEST_UX_PLAN.md). | Playtest 2026-07-03 | S-M | |

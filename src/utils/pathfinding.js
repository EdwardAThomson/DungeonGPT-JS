...
export const findPath = (mapData, start, goal) => {
  ...
  const getCost = (tile) => {
    // Prefer plains, avoid water, slightly penalize forests/mountains
    if (tile.biome === 'water') return 10; // High cost, but not impossible
    if (tile.poi === 'forest') return 2;
    if (tile.poi === 'mountain') return 3;

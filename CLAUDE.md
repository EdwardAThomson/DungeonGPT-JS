...
2. **`cf-worker/`** — the real production backend. Hono + TypeScript on Cloudflare Workers (`dungeongpt-api`). Entry `cf-worker/src/index.ts` mounts route groups: `/api/ai` (text gen via Workers AI binding), `/api/embed` (768-dim BGE embeddings), `/api/image`, `/api/db/*` (Postgres CRUD proxy over the `HYPERDRIVE` binding, the only authed-by-default group), `/api/entitlements`, and `/api/events` (anonymous product-analytics ingestion, #86 — the Worker's only unauthenticated route; protected instead by the CORS origin allowlist, a server-side event-name allowlist, a 1KB props cap, and an IP-keyed rate limit over `request_counters`, see `cf-worker/src/routes/events.ts`). Free-pool AI runs on the `[ai]` Workers AI binding — curated open-weights models (GPT-OSS, Llama, Gemma); **no API keys** needed for free text/embeddings. A member+ premium text pool (`cf-worker/src/services/openrouter.ts`, requested via `pool: 'premium'`) calls OpenRouter and needs the `OPENROUTER_API_KEY` secret; it falls back to the free pool on any failure.
...
Game logic is split:
- **`src/game/`** — controllers and pure engines: `milestoneEngine.js`, `milestoneSpawner.js`, `encounterController.js`, `movementController.js` / `worldMoveController.js`, `saveController.js`, `promptComposer.js`, `ragEngine.js`, `questEngine.js` / `questHints.js` (side quests, load-time backfill, derived hints; `pickOfferableSideQuest` shares one offer path + guardrails between the rumour flow and the town Quest Board), `campaignLauncher.js` / `campaignChain.js` / `prologueComposer.js` (in-save campaign continuation), `codexEngine.js` (discovered-only bestiary/items), `sitePopulator.js` (explorable-site content + loot), `mobMovement.js` (roaming site mobs that chase and force combat), `threat.js` (relative-threat tiers for the site ring + combat badge), `tileWalk.js` (BFS walk-to-any-tile driver for towns/sites), `skillCheck.js` (engine-rolled social/narrative skill checks, #83: parses the model's `[CHECK: skill, tier, target?]` proposal, rolls with the combat modifier stack, and keeps the persisted failed-check lock ledger), `balanceSim.js` (Monte-Carlo balance harness), `heroInvariants.js` / `heroLedger.js` (load-time mechanics healing + append-only grant ledger).
...
- **`useGameInteraction.js`** — builds the milestone prompt context (Active/Completed/Locked with type tags) and strips any leaked completion marker from responses.

All milestones are engine-refereed (#76 Phase 1, 2026-07-19: "engine referees, LLM narrates", see `docs/AI_NARRATION_CONTRACT.md`): the four types (item/combat/location/talk) complete on engine events only, the AI narrates outcomes and never adjudicates them. The old AI-judged `narrative` type and its `[COMPLETE_MILESTONE]`/`[COMPLETE_CAMPAIGN]` marker path are retired; legacy saves' `narrative` milestones are converted to engine types on load (`migrateNarrativeMilestones`, healed in `Game.js`). The `talk` type completes through a "Talk" button on the milestone NPC (`npc_talked` event); authored milestone NPCs (e.g. Captain Ulric) are placed into their quest building by `populateTown` via `getMilestoneNpcsForTown`, and milestone boss fights launch from the POI arrival modal via `getMilestoneBossForTile`. Milestone POI names are injected into the map generator via `customNames` (entries may be size-tagged: `{ name, size }` pins the settlement size), and POIs stay hidden until prerequisites are met (`visibleMilestonePois`). Story templates carrying `customNames` + `milestones` live in `src/data/storyTemplates.js`; premium templates are gated through `src/game/entitlements.js` (tier ladder free < member < premium < elite, fetched once per session from the worker's entitlements endpoint; templates may carry a per-template `minTier`), and server-delivered premium templates are fetched per session via `src/services/premiumContentApi.js` and registered idempotently alongside built-ins.

**Story-template authoring invariants (these fail SILENTLY at runtime, so respect them when adding or editing any campaign in `storyTemplates.js`):**

1. **Quest-item ids must be unique and must never appear in a loot table.** `item` milestones complete purely on `event.itemId === trigger.item` with no location scope (`doesEventMatchTrigger` in `milestoneEngine.js`), so any drop of that id anywhere completes the milestone. Do not reuse a generic id like `map_fragment` for a quest item, and grep the encounter loot tables (`src/data/encounters/*.js`) to confirm the id is not a random drop. A colliding id lets a stray loot drop complete a milestone in the wrong town and permanently skip the authored building scene.
2. **Every milestone POI needs art or it renders as a bare generic flag with no arrival image.** `poiSprite` (`worldTileArt.js`) only has distinctive sprites for a handful of ids; unknown `poi` ids fall through to the shared `milestone()` pennant, and the POI arrival modal shows no image unless the id has a key in `POI_IMAGES` (`worldMoveController.js`). When adding a `type: 'poi'` spawn, either add a sprite case + `POI_IMAGES` entry or explicitly accept the generic art. **Never let a new location ship without deciding this** — flag it to the user.
3. **A milestone's `requires` gates both its POI reveal and its Talk/boss affordances** (`areRequirementsMet` / `computeVisibleMilestonePois`). Empty `requires: []` on a second milestone makes it co-active from turn 1 (this can be intentional parallel objectives; confirm intent rather than assuming a bug). The AI opening only grounds the current town + current milestone, so co-active milestones no longer leak into the opening scene.

A validation test over `STORY_TEMPLATES` should enforce 1 and 2 (unique quest-item ids not present in loot tables; every POI id has registered art or an explicit generic-allowlist entry) so a new campaign cannot regress these silently.

## AI prompt system

`src/data/prompts.js` defines `DM_PROTOCOL` (strict Dungeon Master rules) which wraps every AI prompt; `useGameInteraction.js`'s `generateResponse()` prepends it. `promptComposer.js` builds movement/location narratives. Both backends sanitize responses to strip leaked prompt markers (`[STRICT DUNGEON MASTER PROTOCOL]`, `[TASK]`, etc.) — see `sanitizeResponse` in `src/services/llmService.js` and the equivalent in `cf-worker/src/services/ai.ts`. Milestone context is passed to the model as Active/Completed/Locked with `[item/combat/location/talk]` type tags, and the model is told all outcomes are the engine's: narrate, never adjudicate, emit no completion markers (both sanitize passes plus the client's `cleanAIResponse` strip any leaked `[COMPLETE_...]` marker as defense in depth); active NPC objectives are grounded with the authored NPC's name/venue/personality, and the location context lists the NPCs actually present with a do-not-invent-names instruction. Social/narrative skill checks follow the same contract (#83): the model proposes `[CHECK: skill, tier, target?]` and stops, the engine rolls immediately (`src/game/skillCheck.js`, same modifier stack as combat) and injects the result into the next prompt as `[CHECK RESULT: ...]` fact; failed checks lock per (location, target, skill) in `settings.checkLocks` (cleared by a long rest or entering a different town/site), with spent approaches injected as steering context and re-proposals of a locked check blocked without a roll.
